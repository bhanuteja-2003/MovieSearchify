...
const MovieList = ({ searchQuery, filters, movies }) => {
  ...
  const filteredMovies = movies.filter((movie) => {
    ...
    const matchesDirector = filters.director ? movie.director === filters.director : true;
    const matchesLanguage = filters.language ? movie.language === filters.language : true;
      ...
      {paginatedMovies.map((movie) => (
        <Link to={`/movie/${movie.id}`} key={movie.id} className="movie-card-link">
          <div className="movie-card">
            <img src={movie.thumbnail} alt={movie.name} className="movie-thumbnail" />
            <h3>{movie.name}</h3>
          </div>
        </Link>
      ))}
